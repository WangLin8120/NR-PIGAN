...
k_training_data = gaussian_noise_measure(k_training_data,noise_scale=0.01,noise_scale_high=0.1,with_noise=False)
...
def main2(seed):
    ...
    #np.save(f"results/forward/gaussian/(0.01,0.1)/u_mean_error.npy", u_mean_error)
    #np.save(f"results/forward/gaussian/(0.01,0.1)/k_mean_error.npy", k_mean_error)
    #np.save(f"results/forward/gaussian/(0.01,0.1)/u_std_error.npy", u_std_error)
    #np.save(f"results/forward/gaussian/(0.01,0.1)/k_std_error.npy", k_std_error)
    #np.save("results/forward/gaussian/(0.01,0.1)/NR_PIGAN_time_history", time_history)
    #torch.save(gen, f"results/forward/mix/[0.2,0.4,0.5]/WR_gan_{seed}.pth")
