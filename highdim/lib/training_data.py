...
class SDE():
    ...
    def forward(self, Num, kL, fL, a):
        ...
        rng = np.random.RandomState(22)           # define a random state
...
if __name__ == "__main__":
    ...
    # parser.add_argument('--fl', type=float, default = 1.0)
    ...
    if Case == 'ODE' or 'ODE_khigh' or 'ODE_fhigh' or 'ODE_kfhigh':
        ...
        k_samples, f_samples, u_samples = SDE_data.forward(args.data_size, kL=1.0, fL=0.2, a=1.0)
        np.save(file=r'../database/SDE/u_300.npy', arr=u_samples)
        np.save(file=r'../database/SDE/k_300.npy', arr=k_samples)
        np.save(file=r'../database/SDE/f_300.npy', arr=f_samples)
